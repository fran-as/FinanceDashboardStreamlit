...
def load_static(portfolio_path, cache_path):
    ...
    try:
        ...
        if "symbol" in cache.columns:
            cache = cache.rename(columns={"symbol": "Symbol"})
        cache["Symbol"] = cache["Symbol"].astype(str).str.strip().str.upper()
        cache = cache.drop(columns=["Price", "Previous Close"], errors="ignore")
    except FileNotFoundError:
        cache = pd.DataFrame(columns=["Symbol"])

    return pd.merge(df, cache, on="Symbol", how="left")

if os.path.exists(PORTFOLIO_PATH):
    static_df = load_static(PORTFOLIO_PATH, CACHE_PATH)

    # ❗ Evitar duplicados o símbolos vacíos
    static_df = static_df.dropna(subset=["Symbol"])
    static_df = static_df[static_df["Symbol"].str.upper() != "NAN"]
    static_df = static_df.drop_duplicates(subset=["Symbol"])
    ...
    styled_cols = styled.data.select_dtypes("number").columns
    st.dataframe(
